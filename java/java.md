...
La sfortunata keyword `null` in Java serve a specificare l'assenza di un oggetto. Le variabili dichiarate ma non-inizializzate hanno valore `null`, e il compiler in generale non ci avverte se non le inizializziamo. Inoltre, `null` è assegnabile manualmente a qualunque reference non-primitiva:
...
Il programma compila ed esegue, ma termina subtio lancia la famigerata `NullPointerException` e crasha!
