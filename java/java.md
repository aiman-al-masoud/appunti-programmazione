...
La famigerata keyword `null` in Java serve a specificare l'assenza di un oggetto. Le variabili dichiarate ma non-inizializzate hanno valore `null`, e il compiler in generale non ci avverte. Inoltre, `null` è assegnabile a qualunque reference non-primitiva.
