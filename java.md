...
Conviene pensare ad una reference come a un telecomando. Una reference molto generica, in cui tratto gli oggetti di tipo `Studente`, `Professore` e `FisicoNucleare` tutti da `Persona` astratta, è un telecomando meno potente, ma più semplice da usare, e più generale. 
